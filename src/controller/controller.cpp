...
Controller::Controller() : active_(false), output_position_x_(0), output_attitude_y_(0),
   ...
   */
  controller_parameter_.pid_controller_gain_position_x.proportional = 600;
   ...
   */
  controller_parameter_.attitude_z_gain_open_loop = 30;
   ...
   *  TODO LAB 8 YOUR CODE HERE.
   */
  controller_parameter_.pid_controller_saturation_position_x.input_lower = -0.7;
  controller_parameter_.pid_controller_saturation_position_x.input_upper = 0.5;
...
Controller::setControllerParameter(const ControllerParameter &controller_parameter) {
  ...
  open_loop_controller_attitude_z_.setSaturation(controller_parameter_.open_loop_controller_saturation_attitude_z);
...
Controller::control(const bool &fast_domain) {
  ...
  const auto encoder_data = sensor_->getEncoderData();
  const auto imu_data = sensor_->getIMUData();
  ...
  if (fast_domain) {
    ...
    output_attitude_z_ =
        open_loop_controller_attitude_z_.control() * fabs(encoder_data.velocity_x) + pid_controller_attitude_z_.control();
  }
  ...
  actuation_command_.motor_left_pwm = output_position_x_ + output_attitude_y_ + output_attitude_z_;
  ...
  actuation_command_.motor_right_pwm = output_position_x_ + output_attitude_y_ - output_attitude_z_;
  ...
  actuation_command_.motor_left_forward = actuation_command_.motor_left_pwm >= MotorParameter::pwm_min;
  actuation_command_.motor_right_forward = actuation_command_.motor_right_pwm >= MotorParameter::pwm_min;
  ...
  actuation_command_.motor_left_pwm = clamp(abs(actuation_command_.motor_left_pwm),
                                            static_cast<double>(MotorParameter::pwm_min),
                                            static_cast<double>(MotorParameter::pwm_max));

  actuation_command_.motor_right_pwm = clamp(abs(actuation_command_.motor_right_pwm),
                                             static_cast<double>(MotorParameter::pwm_min),
                                             static_cast<double>(MotorParameter::pwm_max));
}
