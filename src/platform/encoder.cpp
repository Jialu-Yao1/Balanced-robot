...
Encoder::read() {
  ...
  data_.steps = (data_.steps_left + data_.steps_right) / 2;
...
Encoder::calculateVelocity() {
  ...
  data_.velocity_x = low_pass_filter_velocity_x_.filter(
      ((data_.steps - steps_last) / EncoderParameter::steps_per_meter) / PeriodParameter::slow);
...
Encoder::onLeftA() {
  ...
  int pinAState = digitalReadFromISR(ESP32Pin::motor_left_encoder_a);
  int pinBState = digitalReadFromISR(ESP32Pin::motor_left_encoder_b);
...
Encoder::onRightA() {
  ...
  int pinAState = digitalReadFromISR(ESP32Pin::motor_right_encoder_a);
  int pinBState = digitalReadFromISR(ESP32Pin::motor_right_encoder_b);
