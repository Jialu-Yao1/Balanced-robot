...
IOExpander::IOExpander(const uint8_t &address) :
  ...
  mcp23018_->SetDirections(UINT8_MAX, UINT8_MAX);
  ...
  mcp23018_->SetPullups(UINT8_MAX, UINT8_MAX);
...
IOExpander::detachInterruptPortA(const uint8_t &pin) {
  ...
  interrupt_handlers_port_a_[pin] = InterruptHandler();
}
...
IOExpander::detachInterruptPortB(const uint8_t &pin) {
  ...
  interrupt_handlers_port_b_[pin] = InterruptHandler();
}
...
IOExpander::digitalReadPortA(const uint8_t &pin) {
  ...
  auto port_val = static_cast<uint8_t>(mcp23018_->GetPortA());
   ...
   *  TODO LAB 4 YOUR CODE HERE.
   */
  return static_cast<bool>(BIT(pin) & port_val);
}
...
IOExpander::digitalReadPortB(const uint8_t &pin) {
  ...
  auto port_val = static_cast<uint8_t>(mcp23018_->GetPortB());
...
IOExpander::onInterrupt() {
  ...
  auto flag = static_cast<uint16_t>(mcp23018_->readPairFromRegister(INTFA));
  ...
  auto capture = static_cast<uint16_t>(mcp23018_->readPairFromRegister(INTCAPA));
  ...
  uint8_t captureA = capture;
  uint8_t captureB = (capture >> 8);
  ...
  serviceInterrupt(flagA, captureA, interrupt_handlers_port_a_);
  serviceInterrupt(flagB, captureB, interrupt_handlers_port_b_);
}
...
IOExpander::serviceInterrupt(const uint8_t &flags, const uint8_t &captures,
  ...
  for (uint8_t pin = 0; pin < IOExpanderParameter::port_pin_count; pin++, pin_mask <<= 1) {
    ...
    if (!interrupt_handlers.at(pin).handler) {
      continue;
    }
    ...
    switch (interrupt_handlers.at(pin).mode) {
      case RISING: {
        ...
        if ((captures & pin_mask) != 0) {
          interrupt_handlers.at(pin).handler(interrupt_handlers.at(pin).arg);
        }
