...
IMU::read()
{
    ...
    if (!mpu6050_.getEvent(&acceleration, &angular_velocity, &temperature)) {
    	Serial(LogLevel::error) << "Failed to get MPU6050 IMU event.";
    }
...
IMU::initialize()
{
	...
	if (!mpu6050_.begin(firmware::AddressParameter::imu_mpu6050)) {
		Serial(LogLevel::error) << "Failed to initialize MPU6050 IMU driver.";
	}
