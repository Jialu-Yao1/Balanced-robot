...
bestEffortTask() {
  ...
  firmware::Display(0) << "Biped: #" << serial_number_;
  ...
  firmware::Display(1) << "Real-Time: " << execution_time_real_time_task_ << " " << interval_real_time_task_;
  ...
  firmware::Display(2) << "Heap: " << 100. - (ESP.getFreeHeap() * 100. / ESP.getHeapSize()) << "%";
  ...
  if (wifi_ && wifi_->getWiFiStatus() == WL_CONNECTED) {
    firmware::Display(3) << "Wi-Fi: " << wifi_->getWiFiLocalIP();
  } else {
    firmware::Display(3) << "Wi-Fi: disconnected";
  }
  ...
  if (controller_ && controller_->getActiveStatus()) {
    firmware::Display(4) << "Controller: active";
  } else {
    firmware::Display(4) << "Controller: inactive";
  }
  ...
  if (planner_stage < 0) {
    firmware::Display(5) << "Planner: inactive";
  } else {
    firmware::Display(5) << "Planner: stage " << planner_stage;
  }
...
ioExpanderAInterruptServiceTask(void *pvParameters) {
  for (;;) {
    ...
    const uint32_t ulTaskNotifyTakeValue = ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
...
ioExpanderBInterruptServiceTask(void *pvParameters) {
  for (;;) {
    ...
    const uint32_t ulTaskNotifyTakeValue1 = ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
...
networkTask(void *pvParameters) {
  ...
  for (; !wifi_ || wifi_->getWiFiStatus() != WL_CONNECTED;);
  ...
  for (;;) {
    ...
    if (result) {
      ...
      udp_biped_message_->write(NetworkParameter::ip_ground_station,
                                NetworkParameter::port_udp_biped_message,
                                std::string(std::begin(message_serialized), std::end(message_serialized)));
    } else {
      /*
