...
Actuator::Actuator() {
  ...
  if (io_expander_a_) {
    ...
    io_expander_a_->IOExpander::pinModePortA(IOExpanderAPortAPin::motor_left_direction, OUTPUT);
    io_expander_a_->IOExpander::pinModePortA(IOExpanderAPortAPin::motor_right_direction, OUTPUT);
    io_expander_a_->IOExpander::pinModePortB(IOExpanderAPortBPin::motor_enable, OUTPUT);
...
Actuator::actuate(const ActuationCommand &actuation_command) {
  ...
  io_expander_a_->IOExpander::digitalWritePortB(IOExpanderAPortBPin::motor_enable, actuation_command.motor_enable);
  ...
  io_expander_a_->IOExpander::digitalWritePortA(IOExpanderAPortAPin::motor_left_direction,
                                                !actuation_command.motor_left_forward);
  io_expander_a_->IOExpander::digitalWritePortA(IOExpanderAPortAPin::motor_right_direction,
  ...
  int left_motor_value = static_cast<int> (clamp(actuation_command.motor_left_pwm,
                                                 static_cast<double> (MotorParameter::pwm_min),
                                                 static_cast<double>(MotorParameter::pwm_max)));
  int right_motor_value = static_cast<int> (clamp(actuation_command.motor_right_pwm,
                                                  static_cast<double> (MotorParameter::pwm_min),
                                                  static_cast<double>(MotorParameter::pwm_max)));

  analogWrite(ESP32Pin::motor_left_pwm, left_motor_value);
  analogWrite(ESP32Pin::motor_right_pwm, right_motor_value);
